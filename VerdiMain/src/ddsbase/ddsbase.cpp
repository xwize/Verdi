...
void _errorMsg()
{
	VERDI_THROW(Verdi::FileLoadFailedException,"File load failed.",0);
}

#define ERRORMSG() _errorMsg()

// ---- HERE BE DRAGONS ----
// The following code is thoroughly nasty
// but the only way to get PVM support was to include it
...
void DDS_encode(unsigned char *data,unsigned int bytes,unsigned int skip,unsigned int strip,
   ...
   for (i=-128; i<128; i++)
      ...
      lookup[i+128]=(unsigned char)bits;
      }
...
void convfloat(unsigned char *data,unsigned int bytes)
   ...
   for (ptr=data,i=0; i<bytes/4; i++,ptr+=4)
      ...
      data[2*i]=(unsigned char)(ftrc(65535.0f*v+0.5f)/256);
      data[2*i+1]=(unsigned char)(ftrc(65535.0f*v+0.5f)%256);
      }
   }
...
unsigned char *quantize(unsigned char *data,
   ...
   for (k=0; k<depth; k++)
      for (j=0; j<height; j++)
         for (i=0; i<width; i++)
            ...
            data3[i+(j+k*height)*width]=(unsigned short)v;
   ...
   for (k=0; k<depth; k++)
      for (j=0; j<height; j++)
         for (i=0; i<width; i++)
            data2[i+(j+k*height)*width]=(unsigned char)(err[DDS_get(data3,width,height,depth,i,j,k)]+0.5);
